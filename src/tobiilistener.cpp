...
void OnGazeDataEvent(TX_HANDLE hGazeDataBehavior)
{
    ...
    if (txGetGazePointDataEventParams(hGazeDataBehavior, &eventParams) == TX_RESULT_OK) {
        ...
        emit emitter->newGaze(SamplePoint(QPointF(eventParams.X, eventParams.Y), Timer::timestamp()));

    } else {
        emit emitter->newGaze(SamplePoint(QPointF(0, 0), Timer::timestamp()));
...
void OnFixationDataEvent(TX_HANDLE hFixationDataBehavior)
{
    ...
    if (txGetFixationDataEventParams(hFixationDataBehavior, &eventParams) == TX_RESULT_OK) {
        // We will use the current timestamp instead of eventParams.Timestamp
        // With a sing reference for all the timestamps it is easier to work with the events
